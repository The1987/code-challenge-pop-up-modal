...
// Event Listener #1 - When x button is clicked run function 'closeModal' //
document.getElementById("x_button").addEventListener("click", closeModal);

// Event Listener #2 - When user clicks on the grey area (body) run 'closeModal' //
...
function closeModal(event) {
  ...
  if (event.target.id === "wrapper" || event.target.id === "body_id" || event.target.id === "cta-2") {
...
// Event Listener #3 - When hidden container button is clicked run function 'openModal' //
